#include "alg/max_min_fairness.h"
...
class PartialSortAllocator {
 public:
  PartialSortAllocator(int64_t original_capacity, int64_t capacity,
                       int64_t waterlevel,
                       const std::vector<std::vector<int64_t>>& demands,
                       absl::Span<int64_t> sorted_demands)
...
void PartialSortAllocator::PrintState() {
  ...
  std::cerr << "demands: [" << absl::StrJoin(demands_, " ", BpsVecFormatter())
            << "]\n";
  std::cerr << "lower_limit: " << lower_limit_
...
int64_t PartialSortAllocator::ComputeWaterlevel() {
  ...
  while (upper_limit_ >= lower_limit_) {
    ...
    ask += (max_demand_A - waterlevel_) *
           (sorted_demands_.size() - partition_idx - 1);
  ...
  if (next_unsatisfied < sorted_demands_.size()) {
    ...
    waterlevel_ +=
        residual_capacity_ / (sorted_demands_.size() - next_unsatisfied);
  }
...
int64_t SolvePartialSort(int64_t original_capacity, int64_t capacity,
                         int64_t waterlevel,
                         const std::vector<std::vector<int64_t>>& demands,
                         absl::Span<int64_t> sorted_demands) {
  PartialSortAllocator allocator(original_capacity, capacity, waterlevel,
...
int64_t SingleLinkMaxMinFairnessProblem::ComputeWaterlevel(
  ...
  int64_t tiny_demand_thresh =
      capacity / std::max(num_demands, static_cast<int64_t>(1));
  ...
  switch (options_.solve_method) {
    case SingleLinkMaxMinFairnessProblemOptions::kFullSort:
      waterlevel = SolveFullSort(
          capacity, waterlevel, demands,
...
std::ostream& operator<<(
    std::ostream& os, const SingleLinkMaxMinFairnessProblemOptions& options) {
