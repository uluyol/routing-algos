#ifndef ROUTING_ALGOS_ALG_NTH_ELEMENT_H_
#define ROUTING_ALGOS_ALG_NTH_ELEMENT_H_
...
#include "routing-algos/third_party/fastalg/hybrid_qsort.h"
...
void NthElementScratch(RandomIt first, RandomIt nth, RandomIt last,
                       ScratchIt scratch, Compare comp, int depth);

template <typename RandomIt, typename OutIt, typename Compare>
void NthElementInto(RandomIt first, RandomIt nth, RandomIt last, OutIt out,
    ...
    if (p < nth_in_scratch) {
      if (kDebugNthElement) {
        std::cerr << absl::Substitute("call into($0, $1, $2, $3)\n",
                                      p + 1 - scratch, nth - first, n,
                                      (p - scratch) + 1);
      }
      std::move(scratch, p + 1, first);
      NthElementInto(p + 1, nth_in_scratch, scratch + n,
                     first + (p - scratch) + 1, comp, depth);
    } else if (p > nth_in_scratch) {
      if (kDebugNthElement) {
        std::cerr << absl::Substitute("call into($0, $1, $2, $3)\n", 0,
                                      nth - first, p - scratch, 0);
      }
...
void NthElement(RandomIt first, RandomIt nth, RandomIt last, Compare comp) {
  ...
  while (first <= nth && nth <= last) {
    if (last - first > kScratchSize) {
      auto [plo, phi] = exp_gerbens::ChoosePivotAndPartition<kScratchSize>(
          first, last, scratch, comp);
      if (plo < nth) {
        first = phi;
      } else {
        last = plo;
...
#endif  // ROUTING_ALGOS_ALG_NTH_ELEMENT_H_
